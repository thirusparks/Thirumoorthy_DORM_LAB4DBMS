...
INNER JOIN product ON supplier_pricing.PRO_ID = product.PRO_ID) AS Supp_Pro 
INNER JOIN `order` AS O ON O.PRICING_ID = Supp_Pro.PRICING_ID) AS SOPINNER JOIN rating AS R ON R.ORD_ID = SOP.ORD_ID;
END
